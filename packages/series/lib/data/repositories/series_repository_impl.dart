...
class SeriesRepositoryImpl implements SeriesRepository {
  ...
  Future<Either<Failure, List<Series>>> getNowPlayingSeries(
    ...
    if (await networkInfo.isConnected) {
      try {
        final result = await remoteDataSource.getNowPlayingSeries(page: page);
        if (page == 1) {
  ...
  @override
  Future<Either<Failure, List<Series>>> getPopularSeries({int page = 1}) async {
    if (await networkInfo.isConnected) {
      try {
        final result = await remoteDataSource.getPopularSeries(page: page);
        if (page == 1) {
